...
def create_process_diagram(path: Path):
    ...
    control_text = (
        "十部门组织权限｜统一编码与版本｜一码到底｜过程预警｜系统生成单据/报表｜客户与供应商隔离｜"
...
def section_overview(doc):
    ...
    add_bullets(doc, [
        ...
        "公司内部由财务、总经办、生产、品质、后工序、工程、PMC、采购、委外和行政十部门协同，既需要全厂统一数据，又必须明确部门数据责任和权限。",
    ...
    add_numbered(doc, [
        ...
        "为十部门建立独立工作台、责任、审批、报表和预警，使CNC/车床、后工序、独立组装、质量与行政劳务均进入统一数据链。",
    ])
...
def section_acceptance(doc):
    ...
    scenarios = [
        ...
        ("十部门工作台与数据权限", ["分别以财务、总经办、生产、品质、后工序、工程、PMC、采购、委外和行政账号登录。", "验证各部门首页只显示本部门任务、待办、指标和授权数据。", "总经办可分析全厂但不能修改凭证、检验、库存和考勤原始记录。", "采购与委外、生产与后工序、后工序与组装的任务和统计可以区分。", "跨部门交接通过正式单据、状态、责任人和审计完成。"]),
...
REQ_GROUPS = {
    ...
    "ORG": ("组织与部门", [
        "公司工厂部门班组工作中心成本中心利润中心分层", "财务部总经办生产部品质部后工序部工程部PMC采购委外行政十部门", "生产部CNC与车床工作中心分离", "后工序清洗丝印镭雕打磨振磨全检包装工作中心", "组装独立工作中心和独立组装工单", "采购与委外角色单据进度和对账分离", "员工主岗位兼任岗位和权限有效期", "人员技能设备和工序授权", "部门工作台待办任务超期异常和指标", "菜单操作组织行字段状态金额权限", "总经办全厂分析但不可越权修改源数据", "部门数据责任人和指标口径负责人", "跨部门正式单据状态责任人和审计交接", "请假审批代理和到期自动收权", "组织部门岗位工作中心变更保留历史", "部门工作中心成本和产能映射", "业务仓库设备职能即使不独立成部门也有角色", "敏感财务成本价格考勤和个人信息字段隔离", "部门绩效可钻取源单但不改变法定账", "十部门账号权限和跨部门交接UAT"
    ]),
    "OTL": ("订单全生命周期与时效", [
    ...
    "SEC": ("安全与非功能", [
        "角色组织数据字段操作权限", "十部门及工作中心数据范围隔离", "客户供应商行级数据隔离", "敏感价格成本工资考勤个人信息隐藏", "总经办汇总和个人明细权限分离", "多因素认证和账号有效期", "兼任代理权限自动到期", "文件水印下载和登录日志", "接口幂等重试错误队列和监控", "网页平板手机工业终端", "网络中断降级和补传", "备份恢复容灾和日志保留", "主数据单据审批接口审计", "关键删除采用停用或冲销", "性能并发可用性SLA"
    ]),
}
...
def section_appendices(doc):
    ...
    questions = [
        ...
        "请展示财务、总经办、生产、品质、后工序、工程、PMC、采购、委外和行政十部门的独立工作台、数据范围、待办和跨部门交接。",
