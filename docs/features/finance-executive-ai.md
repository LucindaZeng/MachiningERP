...
## 模型选型：DeepSeek V4 Pro

- 问答模型采用 DeepSeek V4 Pro，通过 DeepSeek 官方 API 接入；内部命令行/运维调试可使用 DeepSeek TUI，但业务用户只经过系统内 AI 问答界面。
- 模型仅承担自然语言理解、SQL/语义层查询生成和答案组织；数值一律来自语义层查询结果，禁止模型直接"回忆"数字。
- API Key 由平台统一托管并按环境隔离；请求前对工资、银行账号、个人身份等敏感字段脱敏，返回后在服务端重新拼装。
- 记录每次调用的模型名称与版本、token 用量和费用，纳入 AI 问答审计；模型或提示模板升级前用固定财务问题集回归，防止口径漂移。
- 模型服务不可用或超时时自动降级：提示用户改用标准报表与驾驶舱，不阻塞任何业务流程。
- 后续如更换或增加模型（含私有化部署），按 ADR-0007 评审数据驻留与合规后切换，业务侧接口不变。
